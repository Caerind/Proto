...
public:
	...
	template <typename T>
	static inline constexpr T Hermite(T value) { return value * value * (3 - 2 * value); }
	template <typename T>
	static inline constexpr T Quintic(T value) { return value * value * value * (value * (value * 6 - 15) + 10); }

	template <typename T>
	static inline constexpr T Lerp(T start, T end, T percent) { return start + percent * (end - start); }
	template <typename T>
	static inline constexpr T Bezier(T k, T c) { return c * 2 * k * (1 - k) + k * k; }
